...
interface DealRowProps {
  deal: Deal;
  isPinned?: boolean;
  pinnedIndex?: number;   // B3: rank badge
  isEven?: boolean;
  searchQuery?: string;   // B5: highlight matching text
}
...
const sourceConfig: Record<string, { name: string; bg: string; text: string }> = {
  ...
  lsmaroc:       { name: 'LS Maroc',      bg: 'bg-black',        text: 'text-white' },
...
// B5: highlight matching text in title
function highlight(text: string, query: string): React.ReactNode {
  ...
  return (
    <>
      {text.slice(0, idx)}
      <mark className="bg-yellow-200 text-gray-900 not-italic rounded-sm px-px">{text.slice(idx, idx + query.length)}</mark>
...
export default function DealRow({ deal, isPinned, pinnedIndex, isEven = true, searchQuery }: DealRowProps) {
  const { locale } = useLocale();
  const [saved, setSaved] = useState(() => isSaved(deal.id));
  const [imgErr, setImgErr] = useState(false);
  // B1: hover preview state (fixed-positioned)
  ...
  const timeAgo = useMemo(() => formatTimeAgo(deal.scrapedAt), [deal.scrapedAt]);
  ...
  return (
    <>
      <div className={`group flex items-center gap-2 sm:gap-3 px-2 sm:px-3 py-2 sm:py-2.5 border-b border-gray-100 last:border-0 hover:bg-orange-50/40 transition-colors ${
        isPinned ? 'bg-[#FFFBEB]' : isEven ? 'bg-white' : 'bg-gray-50/70'
      }`}>

        {/* B1: Bigger thumbnail — 56px mobile, 64px desktop */}
        <div
          className="relative flex-shrink-0 w-14 h-14 sm:w-16 sm:h-16 rounded-lg overflow-hidden bg-gray-100 flex items-center justify-center cursor-pointer"
          onMouseEnter={(e) => {
            if (!imageUrl || imgErr) return;
            const rect = e.currentTarget.getBoundingClientRect();
            setPreviewPos({ x: rect.right + 10, y: rect.top + rect.height / 2 - 100 });
          }}
        ...
        {/* Source badge + B2 timestamp */}
        <div className="flex flex-col gap-0.5 flex-shrink-0">
          <span className={`px-1.5 py-0.5 text-[10px] font-bold rounded ${src.bg} ${src.text} whitespace-nowrap`}>
            {src.name}
          </span>
          {timeAgo && (
        ...
        <a href={dealUrl} target="_blank" rel="noopener noreferrer" onClick={handleClick} className="flex-1 min-w-0">
          <span className="block text-xs sm:text-sm text-gray-800 truncate group-hover:text-[#FF5500] transition-colors leading-tight">
            {/* B3: rank badge for pinned */}
            {isPinned && typeof pinnedIndex === 'number' && (
              <span className="mr-1 text-xs">{rankBadge(pinnedIndex)}</span>
            )}
            {/* B5: highlight search query */}
            {searchQuery ? highlight(deal.title, searchQuery) : deal.title}
          </span>
      ...
      {/* B1: Hover image preview (fixed-position, not clipped by overflow) */}
      {previewPos && imageUrl && !imgErr && (
        <div
          className="fixed z-[9999] w-[200px] h-[200px] rounded-xl shadow-2xl overflow-hidden border border-gray-200 pointer-events-none hidden sm:block"
          style={{ left: previewPos.x, top: Math.max(8, previewPos.y) }}
        >
